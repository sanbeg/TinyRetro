...
namespace bomber {
#include "tiny-bomber.h"

uint8_t BOOLREAD(uint8_t);
void BOOLWRITE0(uint8_t);
uint8_t BlockBomb(uint8_t, uint8_t, uint8_t);
uint8_t BombBlitz(uint8_t, uint8_t);
uint8_t COLLISION11(uint8_t, uint8_t, uint8_t, uint8_t, uint8_t, uint8_t, uint8_t, uint8_t);
uint8_t CheckCollisionWithBack(uint8_t, uint8_t, PERSONAGE *);
uint8_t CheckCollisionWithBlock(uint8_t, PERSONAGE *);
uint8_t CheckCollisionWithBomb(uint8_t, PERSONAGE *);
uint8_t CheckLevelEnded(PERSONAGE *);
uint8_t CollisionTiny2Caracter(PERSONAGE *);
void DeadSong(void);
void DestroyBloc(void);
void DestroyEnemy(PERSONAGE *);
uint8_t Explose(uint8_t, uint8_t);
void LoadLevel(uint8_t);
uint8_t PrintLive(uint8_t, uint8_t);
uint8_t RecupeBacktoCompH(uint8_t, PERSONAGE *);
uint8_t RecupeBacktoCompV(uint8_t, PERSONAGE *);
void RefreshCaracter(PERSONAGE *);
void ResetVar(PERSONAGE *);
...
void StartGame(void){
if (INGAME==0) {
 while(1){if (!isPressed(control::BTN_A)){break;}}
INGAME=1;}
}
...
if (isPressed(control::BTN_A)) {StartGame();break;}
}
NEWLEVEL:
if (Level<2) {
if (Level>-1) {
...
//INGAME:
while(1){
if (isPressed(control::BTN_A)) {StartGame();}
if (INGAME) {
if (isPressed(control::BTN_L)){Sprite[0].DirectionV=0;}
else if (isPressed(control::BTN_R)) {Sprite[0].DirectionV=1;}else{if (((Sprite[0].x)%8)==0) Sprite[0].DirectionV=2;}
if (isPressed(control::BTN_D)) {Sprite[0].DirectionH=1;}
else if (isPressed(control::BTN_U)) {Sprite[0].DirectionH=0;}else{if (Sprite[0].Decalagey==0) Sprite[0].DirectionH=2;}
if (isPressed(control::BTN_A)&&(BOMBXY[2]==0)) {
