...
uint8_t PixelAsign(uint8_t);
uint8_t PixelConvert(uint8_t, uint8_t, BALL *);
uint8_t RecupeByte(uint8_t, uint8_t);
uint8_t RecupeFlip(uint8_t, uint8_t);
uint8_t RecupeScreen(uint8_t, uint8_t);
uint8_t RecupeSpring(uint8_t, uint8_t);
uint8_t SelectByte(uint8_t, uint8_t);
void SimulateMove(BALL *);
void SimulateRebounce(uint8_t, BALL *);
uint8_t SliceByte(uint8_t, uint8_t);
void Sound(uint8_t, uint8_t);
void Tiny_Flip(BALL *);
void Tiny_Flip2(BALL *);
void TrimBallOnSpring(BALL *);
void TrimXY(BALL *);
void WriteMove(BALL *);
void WriteMoveBounce(BALL *);
...
while(1){
random(255);
if (((isPressed(control::BTN_D)))||((isPressed(control::BTN_A)))||((isPressed(control::BTN_U)))) {
SSD1306.ssd1306_fillscreen(0x00);
...
if ((ball.y>=29)&&(ball.x>=18)) {if (((isPressed(control::BTN_D)))||(isPressed(control::BTN_A))) {if (SpringBar>54) {SpringBar=SpringBar-2;}}else{if ((SpringBar>SPRINGLONG-10)&&(SpringBar<SPRINGLONG)) {SpringBar=SPRINGLONG;} if (SpringBar<SPRINGLONG) {SpringBar=SpringBar+8;}}}else{SpringBar=SPRINGLONG;}
if ((isPressed(control::BTN_U))) {if (trigFlipG<3) {trigFlipG++;}}else{if (trigFlipG>0) {trigFlipG--;}}
if (((isPressed(control::BTN_D)))||(isPressed(control::BTN_A))) {if (trigFlipD<3) {trigFlipD++;}}else{if (trigFlipD>0) {trigFlipD--;}}
}}
//<<<<<<<<<<<<<<<<<<<MAIN FIN>>>>>>>>>>>>>>>
...
if (totalpush<7) {totalpush++;}else{
 ...
 totalpush=0;
for(uint8_t ttt=60;ttt<240;ttt=ttt+20){beeper.beep(ttt, 6);}
}}
...
beeper.beep(20, 12);
}else{B->SIMSpeedx=1;beeper.beep(1, 6);}return 0;}
...
frameCount++;
if (frameCount>1) {
beeper.beep(1, 20);beeper.beep(20, 20);beeper.beep(1, 20);
bouncePush=0;frameCount=0;}}
FIRSTTIME=0;}
