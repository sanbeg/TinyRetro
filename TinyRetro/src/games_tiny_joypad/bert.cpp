...
uint8_t COLLISION_CHECK(Sprite *);
void DEADIFY(uint8_t, Sprite *);
void DEAD_SOUND(void);
void DIFICULTY_CHANGE(void);
void FLIP_PLATE(void);
void GamePlayUpdate(Sprite *);
uint8_t GridLimitTest(uint8_t, int8_t, int8_t, Sprite *);
uint8_t GridPlate(uint8_t, uint8_t);
uint16_t HighScore(uint8_t, uint8_t, uint8_t, uint8_t);
uint8_t Level_Completed(void);
void LimitCheck(uint8_t, uint8_t, Sprite *);
uint8_t Police_Print(uint8_t, uint8_t);
uint8_t RANDOM(uint8_t);
void RESET_GAME(void);
uint8_t Recupe(uint8_t, uint8_t, Sprite *);
uint8_t RecupeDecalageY(uint8_t);
int8_t RecupeLineY(int8_t);
void Recupe_HighScore(void);
void RenewSprite(uint8_t, Sprite *);
uint8_t Renew_Check_if_Posible(uint8_t, Sprite *);
uint8_t Renew_if_Posible(Sprite *);
void ResetDataVar(Sprite*);
...
void TELEPORT_MAIN(Sprite *);
uint8_t Tiny_Bert_Live_Print(uint8_t, uint8_t);
void Tiny_Flip(uint8_t, Sprite *);
uint8_t UPDATE_LIFT_MAIN(uint8_t, Sprite *);
void UPDATE_MAIN_DEAD(uint8_t, Sprite *);
void UPDATE_MAIN_MOVE(uint8_t, Sprite *);
uint8_t blitzSprite(int8_t, int8_t, uint8_t, uint8_t, uint8_t, const uint8_t *);
void recupe_Score(void);
void refreshJump(uint8_t, Sprite *);
void renew_if_possible_add(uint8_t);
...
MENU_LOOP:;
SEED_RND();
if (isPressed(control::BTN_A)) {Rst_Score();INGAME=1;beeper.beep(150, 200);beeper.beep(100, 200);}else{if (INGAME==0){recupe_Score();}goto FLIP;}
START:;
while(1){
if (CHANGE_LEVEL!=0) {
FLIP_PLATE();
CHANGE_LEVEL--; 
if (CHANGE_LEVEL==0) {goto NEW_LEVEL;}else{beeper.beep(200, 40);BYPASS_DEAD=1;goto FLIP;}
}
...
if (((isPressed(control::BTN_L)))&&(sprite[0].Joypad_Key==0)) {SEED_RND();LimitCheck(0,4,&sprite[0]);}
if (((isPressed(control::BTN_R)))&&(sprite[0].Joypad_Key==0)) {LimitCheck(0,2,&sprite[0]);}
if (((isPressed(control::BTN_D)))&&(sprite[0].Joypad_Key==0)) {LimitCheck(0,3,&sprite[0]);}
if (((isPressed(control::BTN_U)))&&(sprite[0].Joypad_Key==0)) {SEED_RND();LimitCheck(0,1,&sprite[0]);}
...
void DEAD_SOUND(void){
for(uint8_t s=200;s>100;s--){beeper.beep(s, 10);} 
}
