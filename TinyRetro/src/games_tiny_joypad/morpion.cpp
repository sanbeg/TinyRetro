...
namespace morpion {
#include "morpion_spritebank.h"
#include "tiny_joypad_compat.h"

void Assigner_x_Main_TMORPION(uint8_t);
void BLINK_WINNER_TMORPION(void);
uint8_t COMPACT_MAP_TMORPION(uint8_t);
...
uint8_t CPU_TERMINATION_TMORPION(uint8_t);
void Check_WIN_TMORPION(uint8_t);
uint8_t Display_TMORLPION(uint8_t, uint8_t);
void INIT_M_DATA_TMORPION(void);
void NULL_GAME_TMORPION(void);
uint8_t RECUPE_BACK_TMORPION(uint8_t, uint8_t);
uint8_t RECUPE_DOWN_TMORPION(uint8_t, uint8_t);
uint8_t RECUPE_DOWN_X_BOARD_TMORPION(uint8_t);
uint8_t RECUPE_MIDDLE_TMORPION(uint8_t, uint8_t);
uint8_t RECUPE_MIDDLE_X_BOARD_TMORPION(uint8_t);
uint8_t RECUPE_POINTER_TMORPION(uint8_t, uint8_t);
uint8_t RECUPE_POS_GRID_TMORPION(uint8_t);
uint8_t RECUPE_UP_TMORPION(uint8_t, uint8_t);
uint8_t RECUPE_UP_X_BOARD_TMORPION(uint8_t);
void REFRESH_SCREEN_TMORPION(void);
uint8_t Recupe_MENU_TMORPION(uint8_t, uint8_t);
uint8_t Recupe_TMORPION(uint8_t, uint8_t);
void SND_BOX_TMORPION(uint8_t);
void Tiny_Flip_TMORPION(uint8_t, uint8_t, uint8_t);
void Tiny_MENU_TMORPION(uint8_t);
...
uint8_t t;
switch (SND_){
  case 1:for (t=0;t<100;t+=10){beeper.beep(t, 4);}_delay_ms(50); break;
  case 0:for (t=100;t<200;t+=10){beeper.beep(t, 3);}_delay_ms(50);break;
  case 2:COMPLETED_PROCEDURE_TMORPION();break;
  case 3:NULL_GAME_TMORPION();break;
  case 4:for(t=1;t<20;t++){beeper.beep(4, 80);beeper.beep(100, 80);}break;
  case 5:for(t=200;t>10;t--){Sound(200-t,3);beeper.beep(t, 12);}break;
  case 6:beeper.beep(100, 250);beeper.beep(20, 250);break;
  default:break;
}
}
...
pMEM[t]=p[t];}
for (S=0;S<10;S++){
beeper.beep(140, 10);beeper.beep(220, 4);
