...
void SND_TTRIS(uint8_t Snd_TTRIS) {
  ...
  switch (Snd_TTRIS) {
    ...
    case 2: for (q = 1; q < 10; q++) {
        Sound_TTRIS(40, 80);
        Sound_TTRIS(150, 80);
      } break;
    case 3: for (q = 100; q > 10; q--) {
        Sound_TTRIS(q * 2, 6);
        Sound_TTRIS(q, 12);
      } break;
    case 4: Sound_TTRIS(20, 150); Sound_TTRIS(100, 150); break;
    case 5: for (q = 0; q < 126; q++) {
        Sound_TTRIS(q * 2, 1);
      } break;
    default: break;
  }
...
void Tiny_Flip_TTRIS(uint8_t HR_TTRIS) {
  ...
  for (y = 0; y < 8; y++) {
    ...
    for (x = 0; x < HR_TTRIS; x++) {
      SSD1306.ssd1306_send_byte(Recupe_TTRIS(x, y));
    }
...
void Flip_intro_TTRIS(uint8_t *TIMER1) {
  ...
  for (y = 0; y < 8; y++) {
    ...
    SSD1306.ssd1306_send_data_start();
    for (x = 0; x < 128; x++) {
      SSD1306.ssd1306_send_byte(intro_TTRIS(x, y, TIMER1));
    }
    SSD1306.ssd1306_send_data_stop();
  }
}
...
Next_4:;
  if (RESAVE) {
    save_HIGHSCORE_TTRIS(); //resave if sumcheck not pass
  }
}
