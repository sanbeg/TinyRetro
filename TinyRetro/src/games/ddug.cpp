...
void ANIM_UPDATE_MAIN_TDDUG(uint8_t);
void Adjust_main_speed_TDDUG(uint8_t);
uint8_t BACKGROUND_TDDUG(uint8_t, uint8_t);
uint8_t BACK_TDDUG(uint8_t, uint8_t);
...
void LOAD_LEVEL_TDDUG(uint8_t);
uint8_t MAIN_TDDUG(uint8_t, uint8_t);
void NEWGAME_TDDUG(void);
void NEXTLEVEL_TDDUG(void);
uint8_t READ_GRID_TDDUG(int8_t, int8_t);
uint8_t RECUPE_ENEMY_BIG_TDDUG(uint8_t);
uint8_t RECUPE_ENEMY_FRAME_TDDUG(uint8_t);
uint8_t RECUPE_ENEMY_TDDUG(uint8_t, uint8_t);
uint8_t RECUPE_LIVE_TDDUG(uint8_t, uint8_t);
uint8_t RECUPE_START_TDDUG(uint8_t, uint8_t, uint8_t);
uint8_t RECUPE_WEAPON_TDDUG(uint8_t, uint8_t);
void RESET_ALL_GD_TDDUG(void);
void RESET_WEAPON_TDDUG(void);
int8_t RND_TDDUG(void);
uint8_t RecupeDecalageY_TDDUG(uint8_t);
int8_t RecupeLineY_TDDUG(int8_t);
uint8_t Recupe_TDDUG(uint8_t, uint8_t);
void SCORES_ADD_TDDUG(void);
void SND_DDUG(void);
void SND_TDDUG(uint8_t);
uint8_t SPEED_BLITZ_TDDUG(int8_t, int8_t, uint8_t, uint8_t, uint8_t, const uint8_t *);
void Sound_TDDUG(uint8_t, uint8_t);
uint8_t SplitSpriteDecalageY_TDDUG(uint8_t, uint8_t, uint8_t);
void TL_ENEMY_TDDUG(void);
void Tiny_Flip_TDDUG(uint8_t, uint8_t, uint8_t, uint8_t, uint8_t);
int8_t TrackX_TDDUG(int8_t);
int8_t TrackY_TDDUG(int8_t);
void Trigger_adj_TDDUG(void);
void UPDATE_ENEMY_TDDOG(void);
void UPDATE_PANNEL_TDDUG(uint8_t);
uint8_t Universal_TDDUG(int8_t, int8_t, int8_t, int8_t, int8_t, int8_t, int8_t, int8_t);
uint8_t WRITE_GRID_TDDUG(int8_t, int8_t);
uint8_t blitzSprite_TDDUG(int8_t, int8_t, uint8_t, uint8_t, uint8_t, const uint8_t *);
uint8_t recupe_SCORES_TDDUG(uint8_t, uint8_t, int8_t);


const byte MAX_ENEMY_TDDUG = 4;
...
switch(GD_DDUG.Skip_Frame){
      case (0):ADJUST_FRAME_MAIN_DDUG();Tiny_Flip_TDDUG(1,0,109,0,7);
                while((currentMillis-MemMillis)<66){currentMillis=millis();}MemMillis=currentMillis;
...
switch(GamePlay_TDDUG()) {
  case 0:break;
  case 1:Tiny_Flip_TDDUG(1,0,109,0,7);SND_TDDUG(0);DLAY();goto NEXTLEVEL;break;
  default:break;
}
...
case 1:if (((ENEMY_DDUG[GD_DDUG.NOSPRITE_GO_OUT].X()==20)&&(ENEMY_DDUG[GD_DDUG.NOSPRITE_GO_OUT].Y()==0))||(ENEMY_DDUG[GD_DDUG.NOSPRITE_GO_OUT].ACTIVE()==0)) {ENEMY_DDUG[GD_DDUG.NOSPRITE_GO_OUT].PUT_ACTIVE(0);Tiny_Flip_TDDUG(1,0,109,0,7);return 1;}break;
default:break;
}
return 0;
}
...
case(30):ENEMY_DDUG[SPR_].PUT_BIG_ZIP(0);ENEMY_DDUG[SPR_].PUT_ACTIVE(0);Tiny_Flip_TDDUG(1,0,109,0,7);SND_TDDUG(1);W_TDDUG[0].PUT_ACTIVE(5);GD_DDUG.Scores+=5;COMPIL_SCO_TDDUG();Tiny_Flip_TDDUG(1,0,109,7,8);break;
...
EXT_GRID_TDDUG=0;
    for (x = 0; x < Maxx_; x++){
    if (x<19) {SSD1306.ssd1306_send_byte(0x00);goto eend;}
switch(Select_){
...
SSD1306.ssd1306_send_byte(Byte1_);
eend:;
...
void Sound_TDDUG(uint8_t freq, uint8_t dur){
