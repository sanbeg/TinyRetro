...
uint8_t BOOLREAD (uint8_t, uint8_t, SPACE *);
void GRIDMonsterFloorY (SPACE *);
uint8_t LivePrint (uint8_t, uint8_t);
void LoadMonstersLevels (int8_t, SPACE *);
uint8_t Monster (uint8_t, uint8_t, SPACE *);
uint8_t MonsterRefreshMove (SPACE *);
uint8_t MonsterShoot (uint8_t, uint8_t, SPACE *);
void MonsterShootGenerate (SPACE *);
void MonsterShootupdate (SPACE *);
void Monster_Attack_Check (SPACE *);
uint8_t Murge_Split_UP_DOWN (uint8_t, SPACE *);
uint8_t MyShield (uint8_t, uint8_t, SPACE *);
uint8_t MyShoot (uint8_t, uint8_t, SPACE *);
int8_t OuDansLaGrilleMonster (uint8_t, uint8_t, SPACE *);
void RemoveExplodOnMonsterGrid (SPACE *);
uint8_t ShieldBlitz (uint8_t, uint8_t);
uint8_t ShieldDestroy (uint8_t, uint8_t, uint8_t, SPACE *);
void ShieldDestroyWrite (uint8_t, uint8_t, SPACE *, uint8_t);
void ShipDestroyByMonster (SPACE *);
void SnD (int8_t, uint8_t);
void Sound (uint8_t, uint8_t);
void SpeedControle (SPACE *);
uint8_t SplitSpriteDecalageY (uint8_t, uint8_t, SPACE *);
void Tiny_Flip (uint8_t, SPACE *);
void UFOUpdate (SPACE *);
uint8_t UFOWrite (uint8_t, uint8_t, SPACE *);
void UFO_Attack_Check (uint8_t, SPACE *);
void VarResetNewLevel (SPACE *);
uint8_t Vesso (uint8_t, uint8_t, SPACE *);
uint8_t WriteMonster14 (uint8_t);
uint8_t background (uint8_t, uint8_t, SPACE *);
...
Bypass:
        ...
        while(1) {
                ...
                if (ShipDead!=1) {
                  ...
                  if (isPressed(control::BTN_R)) {
                    if (VarPot<108) {VarPot=VarPot+6;}
                  }
                  if (isPressed(control::BTN_A)&&(MyShootReady==SHOOTS)) {
                    Sound(200,4);
                    MyShootReady=0;
                    space.MyShootBall=6;
...
#else
        if (line < 8) {
          space->Shield[BOOLWRITE]=space->Shield[BOOLWRITE]-(1<<(7-line));
           if (Origine==0) {
             space->MyShootBall=-1;
           }
        }
...
uint8_t Murge_Split_UP_DOWN(uint8_t x,SPACE *space){
        ...
        if (space->DecalageY8==0) {
                ...
                if (space->PositionDansGrilleMonsterY==0) {
                        ...
                        if (SpriteType<8) {ANIMs=(space->anim*14);}else{ANIMs=0;}
                        if (SpriteType!=-1) {Murge1=SplitSpriteDecalageY(pgm_read_byte(&Monsters[(WriteMonster14(x-space->MonsterGroupeXpos)+SpriteType*14)+ANIMs]),0,space);}else{Murge1=0x00;}
                        SpriteType=space->MonsterGrid[space->PositionDansGrilleMonsterY][space->PositionDansGrilleMonsterX];
                        if (SpriteType<8) {ANIMs=(space->anim*14);}else{ANIMs=0;}
                        if (SpriteType!=-1) { Murge2=SplitSpriteDecalageY(pgm_read_byte(&Monsters[(WriteMonster14(x-space->MonsterGroupeXpos)+SpriteType*14)+ANIMs]),1,space);}else{Murge2=0x00;}
