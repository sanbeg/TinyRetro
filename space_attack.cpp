...
void ssd1306_fillscreen(unsigned char c) {
...
void ssd1306_char_f6x8(unsigned char x, unsigned char y, const char * str) {
  oled_text::ssd1306_char_f6x8(oled, x, y, str);
}

void doNumber(int x, int y, int n) {
...
void playSpaceAttack() {
  ...
  while (stopAnimate == 0) {
    ...
    // deal with inputs
    if(control::consumePress(control::BTN_A)) fire =1; // this reads the reset pin (pin 1) of the Attiny85 - to use it, see comments section above
